...
parameters = ParameterData(
    dict={
        ...
        'atoms_getters': [
            'temperature',  # translate to "atoms.get_temperature()"
            ['forces', {
                'apply_constraint': True
            }],  # translate to "atoms.get_forces(apply_constraint=True)"
            ['masses', {}],  # translate to "atoms.get_masses()"
        ],
        'calculator_getters': [
            ['potential_energy', {}],  # translate to "calculator.get_potential_energy()"
        ...
        'optimizer': {
            'name': 'QuasiNewton',  # optimizer = QuasiNewton(alpha=0.9)
            'args': {
                'alpha': 0.9
            },
            'run_args': {
        ...
        'pre_lines': [
            '# This is a set',  # a set of lines to be placed in the executed script
            '# of various pre-lines'
        ],  # right after the imports of modules, at the beginning
        'post_lines': ['# This is a set', '# of various post-lines'
                       ],  # a set of lines to be placed in the executed script
        # after the call to optimizers and calculators, before the storage of results
        'extra_imports': [
            'os',  # translate to "import os"
